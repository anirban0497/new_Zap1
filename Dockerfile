...
start_zap() {\n\
    ...
    cd $ZAP_HOME\n\
    ./zap.sh -daemon -host 127.0.0.1 -port 8080 -config api.key=n8j4egcp9764kits0iojhf7kk5 -config api.addrs.addr.name=.* -config api.addrs.addr.regex=true -config api.disablekey=false > /tmp/zap.log 2>&1 &\n\
    ZAP_PID=$!\n\
    ...
    for i in {1..90}; do\n\
        if curl -s "http://127.0.0.1:8080/JSON/core/view/version/?apikey=n8j4egcp9764kits0iojhf7kk5" > /dev/null 2>&1; then\n\
            echo "ZAP is ready!"\n\
            return 0\n\
        fi\n\
...
\n\
# Start Flask app on a different port since ZAP is using 8080\n\
echo "Starting Flask application..."\n\
cd /app\n\
exec python -m gunicorn --bind 0.0.0.0:5000 --timeout 300 --workers 1 app:app' > /app/start.sh \
